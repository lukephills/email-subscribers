...
class es_cls_widget
{
	public static function load_subscription($arr)
	{
		...
		if( $es_name == "YES" ) 
		{
			$es = $es . '<div class="es_lablebox">Name</div>';
		...
		$es = $es . '<div class="es_lablebox">Email *</div>';
		$es = $es . '<div class="es_textbox">';
		...
		$es = $es . '<div class="es_button">';
			$es = $es . '<input class="es_textbox_button" name="es_txt_button_pg" id="es_txt_button_pg" onClick="return es_submit_pages('.$url.')" value="Subscribe" type="button">';
		$es = $es . '</div>';
		if( $es_name != "YES" ) 
		{
